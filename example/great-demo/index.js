$(function(){
	...
	function _updateCurTime(time){
		...
		//添加新的用户 更新已有的用户
		for(var i=0; i<resdata.length; i++){
			var obj = jk.utils.filter(nodes, 'id', resdata[i].id);
			if (obj.length == 0){
				(function(i){
					setTimeout(function() {
	...
	function smallnodeblink(){
		jk.utils.filter(nodes, 'type', 'big', function(obj){
	...
	function tooltip(n, options){
		...
		this.draw = function(){
			var chart = jk.basicChart.generate({
				bindto: '.chart_t',
			    data: {
			        x: 'x',
			        columns: self.pdata
